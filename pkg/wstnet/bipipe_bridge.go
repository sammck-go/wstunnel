...
// BipipeBridger is the interface to a background task that bridges traffic in both directions between two Bipipes. It automatically shuts itself down
// when streams are complete in both directions.
type BipipeBridger interface {
	...
	// edgIndex must be 0 or 1. This method may be called after shutdown is complete to determine the final
	// transfer count.
...
// bufferSize.  The bridge is already activated on return.
...
// The asyncob.Helper wait group is signalled when the goroutine completes.
